...
@app.post("/conversation", response_model=AIMessage)
async def converse(user_message: UserMessage):
    aux = 2
    try:
        sales_agent.human_step(user_message.user_message)
        aux = sales_agent.determine_conversation_stage(aux)
        res = sales_agent._call(user_message.user_message, aux)

        if aux >= 3:
            answer = extract_info(res)
            return AIMessage(ai_message=str(res), extracted_info=answer)
        
        return AIMessage(ai_message=str(res), extracted_info="")
